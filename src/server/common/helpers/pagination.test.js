...
describe('buildPagination', () => {
  it('returns no pagination but a summary for a single page', () => {
    const { pagination, summary } = buildPagination({
      page: 1, perPage: 10, total: 7, totalPages: 1, baseQuery: 'lng=-1.6&lat=55'
    })
  ...
  it('builds items, a next link and a "Showing 1 to 10" summary on page 1', () => {
    const { pagination, summary } = buildPagination({
      page: 1, perPage: 10, total: 657, totalPages: 66, baseQuery: 'lng=-1.6&lat=55'
    })
    ...
    expect(pagination.items[0]).toEqual({ number: 1, href: '?lng=-1.6&lat=55&page=1', current: true })
  })

  it('inserts ellipses for large gaps', () => {
    const { pagination } = buildPagination({
      page: 5, perPage: 10, total: 100, totalPages: 10, baseQuery: 'lng=1&lat=2'
    })
  ...
  it('computes the summary for the last partial page', () => {
    const { summary } = buildPagination({
      page: 66, perPage: 10, total: 657, totalPages: 66, baseQuery: 'lng=1&lat=2'
    })
